...
KEY_LIST = [
    'BID', 'BNAME', 'AUTHOR', 'PUBLICATION_DATE', 'PRESS', 'POSITION', 'SUM',
    'CLASSIFICATION'
]
...
class BookInfo(QGroupBox):
    ...
    def __init__(self, book_msg: dict = None):
        ...
        self.BNAMEInput.mousePressEvent = lambda x: self.inputClick(self.
                                                                    BNAMEInput)
        ...
        self.AUTHORInput.mousePressEvent = lambda x: self.inputClick(
            self.AUTHORInput)
        ...
        self.PRESSInput.mousePressEvent = lambda x: self.inputClick(self.
                                                                    PRESSInput)
        ...
        self.DATEInput.mousePressEvent = lambda x: self.inputClick(self.
                                                                   DATEInput)
        ...
        self.POSITIONInput.mousePressEvent = lambda x: self.inputClick(
            self.POSITIONInput)
        ...
        self.CLASSIFICATIONInput.mousePressEvent = lambda x: self.inputClick(
        ...
        self.btnList = [
            self.BIDInput, self.BNAMEInput, self.AUTHORInput, self.DATEInput,
            self.PRESSInput, self.POSITIONInput, self.NumInput,
            self.CLASSIFICATIONInput
        ]
    ...
    def submitFunction(self):
        ...
        if not self.book_msg['BID'].isdigit():
        ...
        if len(self.book_msg['AUTHOR']) == 0:
            ...
            return
        if len(self.book_msg['PUBLICATION_DATE']) > 7:
            self.errorBox('日期编号过长')
            return
        if len(self.book_msg['PRESS']) > 20:
            self.errorBox('出版社名称长度不能超过20')
            return
        po = self.book_msg['POSITION']
        if len(po) != 3:
            self.errorBox('位置编号长度不为3')
            return
        elif not po[0].isalpha():
            self.errorBox('位置编号首字母应为字母')
            return
        elif not po[0].isupper():
            self.errorBox('位置编号首字母未大写')
            return
        elif not po[1:].isdigit():
            self.errorBox('位置编号后两位应是数字')
            return
        if self.book_msg['SUM'].isdigit():
            if int(self.book_msg['SUM']) <= 0:
                self.errorBox('图书数量小于 1')
                return
            self.book_msg['SUM'] = int(self.book_msg['SUM'])
        else:
            self.errorBox('图书数量有非法字符')
            return
        if self.book_msg["PUBLICATION_DATE"][4] != '-':
            self.errorBox('日期格式不对')
            return
        all_num = self.book_msg["PUBLICATION_DATE"][:4] + self.book_msg[
            "PUBLICATION_DATE"][5:]
...
if __name__ == '__main__':
    book_msg = {
        ...
        'BNAME': '打狗棍法',
        'AUTHOR': '不知名的丐帮帮主',
        'PUBLICATION_DATE': '1206-12',
        'PRESS': '中华出版社',
        'POSITION': 'A01',
        'SUM': 50,
        'CLASSIFICATION': '棍法 丐帮'
    }
